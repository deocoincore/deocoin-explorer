...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(DeocoinCoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidDeocoinAddress: function (address) {
                return DeocoinCoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = DeocoinCoreLib.crypto.Hash.sha256sha256(new DeocoinCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return DeocoinCoreLib.encoding.Base58.encode(new DeocoinCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = DeocoinCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new DeocoinCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = DeocoinCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new DeocoinCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = DeocoinCoreLib.crypto.Hash.sha256ripemd160(new DeocoinCoreLib.deps.Buffer(addr, 'hex'));
